...
class TestMailingsModels:
    ...
    def test_mailing_status_started(self):
        ...
        mailing = Mailing.objects.create(first_sent_at=start, end_at=end, message=message, is_sent=True)
        mailing.recipients.add(client)
...
class TestMailingsViews:
    def test_client_create_view(self, client):
        ...
        response = client.post(url, {"email": "new@test.com", "full_name": "New User", "comment": "Test"})
    ...
    def test_mailing_update_view(self, client):
        ...
        mailing = Mailing.objects.create(
            first_sent_at=timezone.now() + timedelta(days=1), end_at=timezone.now() + timedelta(days=7), message=message
        )
...
class TestMailingsCommands:
    def test_command_send_mailing(self):
        ...
        message = Message.objects.create(subject="Subj", body="Body")
        mailing = Mailing.objects.create(
            first_sent_at="2026-06-10 10:00:00", end_at="2026-06-20 10:00:00", message=message
        )
        mailing.recipients.add(client)
        out = StringIO()
